    ...
    if (file) {
        ...
        reader.onload = async (loadEvent) => {
            ...
            try {
                ...
                // Acessa o primeiro item do array predictions
                if (jsonResponse && jsonResponse.predictions && jsonResponse.predictions.length > 0) {
                    const prediction = jsonResponse.predictions[0];
                    const { chunks } = prediction;
...
async function submitInput() {
    ...
    try {
        ...
        if (jsonResponse && jsonResponse.predictions && jsonResponse.predictions.length > 0) {
            const prediction = jsonResponse.predictions[0];
            const { output, history, prompt, chunks } = prediction;

            setModelResponse(output || "No output provided by the model.");
            setInteractionHistory(Array.isArray(history) ? history.join('\n') : "No interaction history.");
...
async function submitCustomPrompt(yourPrompt) {
    ...
    try {
        ...
        if (jsonResponse && jsonResponse.predictions && jsonResponse.predictions.length > 0) {
            const prediction = jsonResponse.predictions[0];
            // prompt agora deve ser acessado a partir do primeiro item do array
            setPromptEngineering(prediction.prompt || "Prompt send");
        } else {
            console.error('Unexpected JSON response format:', jsonResponse);
        }
...
async function fetchPrompt() {
    try {
        ...
        if (jsonResponse && jsonResponse.predictions && jsonResponse.predictions.length > 0) {
            const prediction = jsonResponse.predictions[0];
            // caso o campo seja prompt_template, acesse prediction.prompt_template
            setYourPrompt(prediction.prompt_template || "");
            showModal();  
        } else {
            console.error('Unexpected JSON response format:', jsonResponse);
        }
		...
		<div className="header">
			...
			<div className='title-info'>
				<div className="header-title">
					<h3 className='title'>Chatbot: Galileo Protect with Observer</h3>
				</div>
							...
							content={
								<div className='outer-padding'>
									...
									<div className="output-info">
										{ showLlmInfo &&	
											<p>
												This is where the actual large language model is run. Based on the input prompt and generates an output one word at a time.
											</p>
										}
					...
					content={
						<div>
							<div className='title-with-icon' style={{display:"flex", justifyContent:"center"}}>
								<h4>Observe and Protect your GenAI applications </h4>
								<div className='title-with-icon-icon'>
									{ showBlackBoxInfo ? 
							...
							<div>
								{ showBlackBoxInfo &&
									<p>
										This module represents the LLM response
									</p>
								}
							</div>
							<div className='outer-padding'>
